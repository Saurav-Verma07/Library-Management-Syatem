...
import { BookA, DiamondPercent, NotebookPen, Trash2 } from "lucide-react";
...
const BookManagement = () => {
  ...
  const { addBookPopup, readBookPopup, recordBookPopup, deleteBookPopup,
  bookIdToDelete } = useSelector(
    (state) => state.popup
  );
    ...
    <>
      <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
          <h2 className="text-xl font-medium md:2xl md:font-semibold">
          ...
          <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
            {isAuthenticated && user?.role === "Admin" && (
              ...
              >
                <span className="flex bg-white justify-center items-center overflow-hidden rounded-full text-black w-[25px] h-[25px] text-[27px] absolute left-5">
                  +
                </span>
                Add New Books
              </button>
            ...
            <tbody>
              {searchedBooks.map((book, index) => (
                ...
                >
                  ...
                  {isAuthenticated && user?.role === "Admin" && (
                    <td className="px-4 py-2 flex space-x-2 m-3 justify-center "><BookA onClick={()=>openReadPopup(book._id)}/>
                    <NotebookPen onClick={()=>openRecordBookPopup(book._id)}/>
                    <Trash2 className="text-red-500 cursor-pointer" onClick={()=>dispatch(toggleDeleteBookPopup(book._id))
