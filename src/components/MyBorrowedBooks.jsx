    ...
    <>
      <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center ">
          <h2 className="text-xl font-medium md:text2xl md:font-semibold">
            Borrowed Books
          </h2>
        </header>
        ...
        <header className="flex flex-col gap-3 sm:flex-row  md:items-center ">
          ...
          <button
            className={`relative rounded sm:rounded-tl-none sm:rounded-br-none sm:rounded-tr-lg sm:rounded-br-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${
              filter === "nonReturned"
                ? "bg-black text-white border-black"
                : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"
            }`}
              ...
              <tbody>
                {booksToDisplay.map((book, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{formatDate(book.borrowDate 
                      
                    )}</td>
                    <td className="px-4 py-2">{formatDate(book.dueDate)}</td>
                    <td className="px-4 py-2">{book.returned ? "Yes" : "No"}</td>
                    <td className="px-4 py-2"><BookA onClick={() => openReadPopup(book.bookId)}/>
                    </td>
                  </tr>
                ))}
              </tbody>
...
<h3 className="text-3xl mt-5 font-medium">No Returened Books Found !

</h3>

        ):(  
       <h3 className="text-3xl mt-5 font-medium">No Non-Returened Books Found !</h3>
      )}
      </main>
